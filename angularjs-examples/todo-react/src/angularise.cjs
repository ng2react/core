// From: https://github.com/xjpro/react-in-angularjs
          ...
          this.$onInit = () => {
            for (let bindingKey of Object.keys(bindings)) {
              ...
              if (bindings[bindingKey] === "=") {
                previous[bindingKey] = cloneDeep(this[bindingKey]);
              }
            }
          };

          this.$doCheck = () => {
            console.log('Checking', previous, this)
            for (let previousKey of Object.keys(previous)) {
              if (!equals(this[previousKey], previous[previousKey])) {
                this.$onChanges();
                previous[previousKey] = cloneDeep(this[previousKey]);
                return;
              }
            }
          };
